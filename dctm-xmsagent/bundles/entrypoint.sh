...
[ -d /var/xms ] && export XMS_DATA_DIR=/var/xms

[ -z "$MEM_XMSX" ] && MEM_XMSX=2048m

CATALINA_OPTS="${CUSTOM_CATALINA_OPTS} -Xmx${MEM_XMSX} ${CATALINA_OPTS}"
JAVA_OPTS="${CUSTOM_JAVA_OPTS} -DXMS_DATA_DIR=${XMS_DATA_DIR} ${JAVA_OPTS}"
...
rm -rf ${CATALINA_HOME}/temp && mkdir ${CATALINA_HOME}/temp
rm -rf ${CATALINA_HOME}/work && mkdir ${CATALINA_HOME}/work
