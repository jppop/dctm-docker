...
usage() {
    ...
    cat 2>&1 <<EOF
usage: `basename $0` [--environment env] [--deployment-method method] [--xplore-idx]
					 --war-file warfile --configuration-file configuration

Deploy a xCP application (a war and a configuration file generated by xCP Designer)

Options:
env               is the xMS environment (default DEV-DX)
deployment-method is the deployment method. Default is full (see xMS deployment guide)
xplore-idx        enable xPlore indexing (disabled by default)
EOF
    exit 1
}
